'use strict';
/* Sprint Velocity — the offline service worker.
   ==========================================================================
   ...
   2. CACHE STORAGE IS ORIGIN-WIDE, NOT PER APP. Every app in the family shares
      ...
      (financial-plan's `fin-shell-`, Flow Metrics' `td-shell-`) sit in the same
      store. The answer is the rule below: only files that are ALREADY PUBLIC in
      this repo are ever cached. Nothing an attacker could read from the cache
      is anything they couldn't read from GitHub. The sprint figures stay in
   ...
   The scope is this file's own directory. Widening it past `/sprint-velocity/`
   ...
   running here, the app refuses to open it rather than letting sanitizeIds()
   strip the fields it has never heard of and push the loss back to the cloud.
...
const CACHE = 'sv-shell-v2';
const PREFIX = 'sv-shell-';
...
/* THE ALLOWLIST, and the security boundary of this file. Every entry is a file
   ...
   the sprint data, not tests.html, not the repo's own
...
/* Resolved against this file's own URL, so the same list works unchanged on
   ...
   /sprint-velocity/). Don't hard-code the directory. */
...
self.addEventListener('activate', (e) => {
  e.waitUntil((async () => {
    /* ONLY this app's caches. `caches.keys()` is origin-wide, so an unguarded
       "delete everything that isn't mine" here would wipe Flow Metrics' or
