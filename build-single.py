...
"""Build the single-file, offline copy of the app.
    ...
    python3 build-single.py            -> dist/sprint-predictability.html
...
OUT_NAME = 'sprint-predictability.html'
REPO = 'sprint-velocity'
APP_TITLE = 'Sprint Predictability'
...
def build():
    ...
    doc.cut_between("F('shareSelectAll').addEventListener('click', () => {",
                    'let shareSeq = 0;', 'drop Select All wiring')
    doc.cut_between("F('shareBtn').onclick = openShare;",
                    '/* ═══════════════════════════════ Sample data',
                    'drop Share wiring')
    doc.sub("  F('shareBtn').hidden = nothingRecorded;\n", '', 1,
            'drop Share in header render')
    doc.sub("const shareDialog = F('shareDialog');",
            "/* Detached: the window itself is not in this build, and the only\n"
            "   thing still holding this is a backdrop handler that can never\n"
            "   fire. */\nconst shareDialog = document.createElement('dialog');",
            1, 'detach Share window ref')
    doc.sub("['manageBtn', 'dataBtn', 'shareBtn', 'targetsBtn']",
            "['manageBtn', 'dataBtn', 'targetsBtn']", 1, 'drop Share from viewOnly')
