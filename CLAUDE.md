...
- **The share window groups its team list by ART (2026-09-14), and a heading is a shortcut, never a selection.** Asked for with twenty-four teams in one column. `shareTeamGroups()` heads each ART's teams with its name (ARTs in their own order, teams on no ART last under "No ART", null — and the plain list — when no team is on an ART), and the heading is a tick box that picks or clears its teams. `sharePicked()` and `shareBoxes()` read `input[data-share-team]` ONLY: before this they read every `input` in the list, which would have put an ART id into the payload and an ART into the "N of M selected" count. The heading's checked/indeterminate state is worked out from its teams at the end of `syncSelectAll()`, which every path that moves a team box ends in. The 24px indent is measured (the UA gives a checkbox 4px before and 3px after). **Flow Metrics carries the identical block** — change both.

- **Nothing below the header is painted until the first `render()` has run — `data-booting` (2026-09-12).** `#views` is empty in the markup — every card is built by `render()`. The browser paints the parsed body before the main script executes, so first paint was the header and then the **footer immediately under it**: the privacy line — "…sign in with Google…" — flashing at the top of the page on every refresh, before the app appeared and shoved it back down. Charles saw it in Money Map; a sweep of all eight pages on 2026-09-12 found **five** doing it (Money Map, Sprint Velocity, League Night, PAPTrack and the `claude-starter` template) and three clean (Golf Handicap, Flow Metrics, the NY calculator — clean because their cards are REAL MARKUP inside `<main>`). The fix is the arrangement the head script already uses for the theme: a thing settled a frame late is a page that visibly jumps as it opens.
...
- **THE ART PICKER TAKES SEVERAL ARTs (2026-09-08), AND `settings.artFilter` IS A LIST.** Asked for by Charles the day after the same change landed in Flow Metrics, which is the reference implementation — same class names, same `.checkline` menu, same "All ARTs is the RESET rather than a sixth option", same border-triangle caret. The `<select>` could say ONE ART or EVERY team and nothing between, so wanting two of five meant taking every team you support. What is different HERE, and it is the whole reason this was raised with Charles rather than mirrored quietly:
  - **The value lives in `settings`, not in a `view` object** — saved, shared, and crossing `sanitizeIds()`. So this needed a boundary change Flow Metrics did not: a new `keepKnown` kind, **`idList`**, which rebuilds the list keeping only entries that pass `ID_OK` or are `ART_NONE`, widens a pre-2026-09-08 STRING to a list of one, and drops the key outright if what arrived was not a list at all. `cleanKey` became a per-entry `cleanArt`, because the single-value version skipped `clean()` for the WHOLE value when it was the sentinel — in a mixed list that is always wrong one way or the other.
  - **`idList` is OUT of the general repair count**, beside `targets` and for the same reason: a rebuilt array can never be `===` the one that came in, so leaving it in would have called every boot a scrub and `save()`d on each one. That is the `numOrBlank` fault (2026-09-01) one field over, and it is pinned by a test.
